...
func TestPullGetPRFileDiff(t *testing.T) {
	...
	client, _ := New("https://api.bitbucket.org")
	got, _, err := client.PullRequests.GetPRFileDiff(context.Background(), "atlassian/atlaskit", 1, "CONTRIBUTING.md")
	if err != nil {
		t.Error(err)
	}
	if got == nil {
...
func TestPullGetPRFileDiff_NotFound(t *testing.T) {
	...
	client, _ := New("https://api.bitbucket.org")
	got, _, err := client.PullRequests.GetPRFileDiff(context.Background(), "atlassian/atlaskit", 1, "missing.md")
	if err != nil {
		t.Error(err)
	}
	if got != nil {
