...
func (s *pullService) GetPRFileDiff(ctx context.Context, repo string, prNumber int, filePath string) (*scm.Change, *scm.Response, error) {
	...
	statPath := fmt.Sprintf("2.0/repositories/%s/pullrequests/%d/diffstat?path=%s", repo, prNumber, escaped)
	out := new(diffstats)
	...
	diffPath := fmt.Sprintf("2.0/repositories/%s/pullrequests/%d/diff?path=%s", repo, prNumber, escaped)
