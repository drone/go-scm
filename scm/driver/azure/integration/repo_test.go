...
func TestListHooks(t *testing.T) {
	...
	_, response, err := client.Repositories.ListHooks(context.Background(), repositoryID, &scm.ListOptions{})
	if err != nil {
		t.Errorf("List got an error %v", err)
	}
	if response.Status != http.StatusOK {
		t.Errorf("List did not get a 200 back %v", response.Status)
	}
...
func TestCreateDeleteHooks(t *testing.T) {
	...
	originalHooks, _, _ := client.Repositories.ListHooks(context.Background(), repositoryID, &scm.ListOptions{})
	// create a new hook
	...
	outHook, createResponse, createErr := client.Repositories.CreateHook(context.Background(), repositoryID, inputHook)
	if createErr != nil {
	...
	afterCreate, _, _ := client.Repositories.ListHooks(context.Background(), repositoryID, &scm.ListOptions{})
	...
	deleteResponse, deleteErr := client.Repositories.DeleteHook(context.Background(), repositoryID, outHook.ID)
	if deleteErr != nil {
	...
	afterDelete, _, _ := client.Repositories.ListHooks(context.Background(), repositoryID, &scm.ListOptions{})
