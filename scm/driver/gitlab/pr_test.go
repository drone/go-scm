...
func TestPullEditComment(t *testing.T) {
	...
	gock.New("https://gitlab.com").
		Put("/api/v4/projects/diaspora/diaspora/merge_requests/1/notes/1").
		MatchParam("body", "lgtm").
		Reply(200).
		Type("application/json").
		SetHeaders(mockHeaders).
	...
	client := NewDefault()
	got, res, err := client.PullRequests.EditComment(context.Background(), "diaspora/diaspora", 1, 1, input)
	if err != nil {
		t.Error(err)
		return
	}
...
func TestPullGetPRFileDiff(t *testing.T) {
	...
	client := NewDefault()
	got, _, err := client.PullRequests.GetPRFileDiff(context.Background(), "diaspora/diaspora", 1347, "VERSION")
	if err != nil {
		t.Error(err)
		return
	}
...
func TestPullGetPRFileDiff_NotFound(t *testing.T) {
	...
	client := NewDefault()
	got, _, err := client.PullRequests.GetPRFileDiff(context.Background(), "diaspora/diaspora", 1347, "does/not/exist.go")
	if err != nil {
		t.Error(err)
		return
	}
...
func TestPullGetPRFileDiff_Paginated(t *testing.T) {
	defer gock.Off()
	...
	client := NewDefault()
	got, _, err := client.PullRequests.GetPRFileDiff(context.Background(), "diaspora/diaspora", 1347, "late_file.go")
	if err != nil {
		t.Error(err)
		return
	}
