...
type (
	...
	PullRequestService interface {
		...
		// the pull request's changes. Pagination over the pull request's changes is
		// handled internally.
		GetPRFileDiff(ctx context.Context, repo string, prNumber int, path string) (*Change, *Response, error)
